...
void demo() { //测试脚本的实现
	...
	while((ch = infile2.get()) != EOF) { //通过换行符获取指令的数量
		if (ch == '\n') {
			numofsum++;
		}
	}
	...
	for (int j = 0; j < numofsum; j++) {
		cout << endl << "第" << j + 1 << "条指令：" << endl;
		s1 = s2 = s3 = s4 = s5 = s6 = s7 = s8 = s9 = false; //初始化
		num = sum = 0;
		while ((ch = infile.get()) != EOF) { //对输入进行检测，并且通过回车结束输入
			if (ch == ' ' || ch == '\t') {
				continue;
			}
			else if (ch == '-') {
				...
				else if (ch == 'z') { //调用exe程序，调用成功通过语言提示，不直接调用
					cout << "调用exe成功！" << endl;
					break;
				}
			}
...
void main() {
	char ch;
	char name[10][1000]; //储存文件名
	bool s1, s2, s3, s4, s5, s6, s7, s8, s9, s10; //保存每个功能的状态
	s1 = s2 = s3 = s4 = s5 = s6 = s7 = s8 = s9 = s10 = false; //初始化
	int sum = 0, i;
	num = 0;
